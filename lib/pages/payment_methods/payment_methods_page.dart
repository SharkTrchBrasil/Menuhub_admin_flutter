                ...
                AppPrimaryButton(
                  ...
                  onPressed: () async {
                    DialogService.showPaymentDialog(
                      context,
                      widget.storeId,
                      onSaved: (_) {}, // Atualização via socket
                    );
                  },
                ),
              ],
  ...
  Widget firstcontain({required double size}) {
    ...
    return BlocBuilder<StoresManagerCubit, StoresManagerState>(
      builder: (context, state) {
        final store = context.read<StoresManagerCubit>().getStoreById(widget.storeId);
        final methods = store?.store.paymentMethods ?? [];
                ...
                child: Center(
                  child: ListTile(
                    ...
                    leading: CircleAvatar(
                      ...
                      radius: 24,
                      child: Icon(_getPaymentIcon(method.paymentType), color: color),
                    ),
                    title: Text(
                      method.customName,
                    ...
                    subtitle: Column(
                      ...
                      children: [
                        if (method.taxRate != 0)
                          Text('Taxa: ${method.taxRate.toStringAsFixed(2)}%', style: const TextStyle(fontSize: 13)),
                        if (method.paymentType.toLowerCase() == 'pix' && method.pixKey?.isNotEmpty == true)
                          Text('Chave Pix: ${method.pixKey}', style: const TextStyle(fontSize: 13)),
                      ],
                    ),
                    trailing: method.paymentType != 'Cash'
                      ...
                      onSelected: (value) async {
                        ...
                        //   final result = await context.read<StoresManagerCubit>().deletePaymentMethod(
                        //     store!.store.id!,
                        //     method.id!,
                        //   );
                        //   result.fold(
                        //         (error) => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Erro ao deletar'))),
                        //         (_) => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Deletado com sucesso'))),
                        //   );
                        // }
                      },
