...
class OrderCubit extends Cubit<OrderState> {
  ...
  void _subscribeToStoresManager(StoresManagerCubit storesManagerCubit) {
    _storesManagerSubscription?.cancel();
    _storesManagerSubscription = storesManagerCubit.stream.listen((state) {
      if (state is StoresManagerLoaded) {
        _subscribeToOrdersForStore(state.activeStoreId, storesManagerCubit);
  ...
  Future<void> _onOrdersReceived(List<OrderDetails> newOrders, int storeId, StoresManagerCubit storesManagerCubit) async {
    await _checkForPendingPrints(newOrders, storesManagerCubit);
  ...
  Future<void> _checkForPendingPrints(List<OrderDetails> orders, StoresManagerCubit storesManagerCubit) async {
    print('[OrderCubit] Verificando ${orders.length} pedidos por impressões pendentes...');
    int manualPrintCounter = 0;

    for (final order in orders) {
      final pendingJobs = order.printLogs.where((log) => log.status == 'pending').toList();
      ...
      if (pendingJobs.isNotEmpty) {
        ...
        if (store != null && store.relations.storeOperationConfig!.autoPrintOrders) {
          ...
          final printPayload = PrintJobPayload(
            jobs: pendingJobs.map((job) => PrintJob(id: job.id, destination: job.printerDestination)).toList(),
            orderId: order.id,
          );
  ...
  Future<void> reprintOrder(OrderDetails order, StoresManagerCubit storesManagerCubit) async {
    print('[OrderCubit] Iniciando reimpressão para o pedido #${order.id}');
    final jobsToReprint = order.printLogs.where((log) => log.status == 'pending' || log.status == 'failed').toList();
    ...
    if (store != null) {
      final printPayload = PrintJobPayload(
        jobs: jobsToReprint.map((job) => PrintJob(id: job.id, destination: job.printerDestination)).toList(),
        orderId: order.id,
      );
      await _printManager.processPrintJobs(printPayload, order, store);
    } else {
  ...
  Future<void> close() {
    ...
    _ordersSubscription?.cancel();
    _connectionSubscription?.cancel();
    print('[OrderCubit] Cubit fechado e subscriptions canceladas.');
    return super.close();
  }
}